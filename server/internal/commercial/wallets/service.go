...
func (s *Service) QuoteManagedNumberPurchase(ctx context.Context, organizationID uuid.UUID) (uuid.UUID, int64, string, error) {
	...
	if err != nil {
		return uuid.Nil, 0, "", fmt.Errorf("resolve managed number offer: %w", err)
	}
	if !plan.Active {
		return uuid.Nil, 0, "", ErrManagedNumberPriceUnavailable
	}
	...
	for i := range prices {
		...
		if price.PricingType != catalog.PricingTypeOneTime || currency == "" || price.AmountMinor == nil || *price.AmountMinor <= 0 {
			continue
		}
		if matched != nil {
			return uuid.Nil, 0, "", ErrManagedNumberPriceUnavailable
