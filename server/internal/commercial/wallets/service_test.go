...
func TestIncreaseToRejectsInvalidMoneyBeforeDatabaseAccess(t *testing.T) {
	service := NewService(nil)
	tests := []IncreaseReservationInput{
		{TargetAmountMinor: 0, ExpiresAt: time.Now().Add(time.Hour)},
		{TargetAmountMinor: 100, ExpiresAt: time.Now().Add(-time.Hour)},
	}
	for _, input := range tests {
		_, err := service.IncreaseTo(context.Background(), uuid.New(), uuid.New(), input)
		if !errors.Is(err, ErrInvalidMoney) {
			t.Fatalf("IncreaseTo() error = %v, want %v", err, ErrInvalidMoney)
		}
	}
}
