...
func TestIssueCreatesTenantBoundExpiringCoturnCredential(t *testing.T) {
	...
	got, err := service.Issue(organizationID)
	if err != nil {
...
func TestIssueReturnsUniqueCredentials(t *testing.T) {
	...
	first, err := service.Issue(organizationID)
	if err != nil {
		t.Fatalf("issue first credential: %v", err)
	}
	second, err := service.Issue(organizationID)
	if err != nil {
...
func TestNewServiceRejectsUnsafeConfiguration(t *testing.T) {
	tests := []struct {
		...
		{name: "missing secret", config: Config{URLs: []string{"turn:turn.example.com"}, TTL: time.Minute}},
		{name: "short secret", config: Config{AuthSecret: "secret", URLs: []string{"turn:turn.example.com"}, TTL: time.Minute}},
		{name: "missing URLs", config: Config{AuthSecret: strings.Repeat("s", 32), TTL: time.Minute}},
		{name: "insecure URL scheme", config: Config{AuthSecret: strings.Repeat("s", 32), URLs: []string{"https://turn.example.com"}, TTL: time.Minute}},
		{name: "missing URL target", config: Config{AuthSecret: strings.Repeat("s", 32), URLs: []string{"turn:?transport=udp"}, TTL: time.Minute}},
		{name: "zero TTL", config: Config{AuthSecret: strings.Repeat("s", 32), URLs: []string{"turn:turn.example.com"}}},
		{name: "excessive TTL", config: Config{AuthSecret: strings.Repeat("s", 32), URLs: []string{"turn:turn.example.com"}, TTL: 25 * time.Hour}},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if _, err := NewService(tt.config); err == nil {
...
func TestIssueRejectsMissingOrganization(t *testing.T) {
	service, err := NewService(Config{AuthSecret: strings.Repeat("s", 32), URLs: []string{"turn:turn.example.com"}, TTL: time.Minute})
	if err != nil {
		t.Fatalf("new service: %v", err)
	}
	if _, err := service.Issue(uuid.Nil); err == nil {
