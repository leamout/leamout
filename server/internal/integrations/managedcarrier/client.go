...
const (
	availableNumbersPath = "/internal/v1/managed-carrier/numbers/available"
	numberOrdersPath     = "/internal/v1/managed-carrier/number-orders"
)
...
func NewClient(cfg Config) (*Client, error) {
	...
	if baseURL == "" {
		return nil, fmt.Errorf("managed carrier base URL is required")
	}
	parsed, err := url.Parse(baseURL)
	if err != nil {
		return nil, fmt.Errorf("parse managed carrier base URL: %w", err)
	}
	if parsed.Scheme != "https" && parsed.Scheme != "http" {
		return nil, fmt.Errorf("managed carrier base URL must use http or https")
	}
	token := strings.TrimSpace(cfg.Token)
	if token == "" {
		return nil, fmt.Errorf("managed carrier token is required")
	}
	deploymentID := strings.TrimSpace(cfg.DeploymentID)
	if deploymentID == "" {
		return nil, fmt.Errorf("managed carrier deployment ID is required")
	}
...
func (c *Client) doJSON(ctx context.Context, method, path string, requestBody any, responseBody any) error {
	if c == nil || c.baseURL == nil || c.httpClient == nil {
		return fmt.Errorf("managed carrier client is not initialized")
	}
	body, err := json.Marshal(requestBody)
	if err != nil {
		return fmt.Errorf("encode managed carrier request: %w", err)
	}
	endpoint := *c.baseURL
	...
	if err != nil {
		return fmt.Errorf("create managed carrier request: %w", err)
	}
	...
	if err != nil {
		return fmt.Errorf("managed carrier request failed: %w", err)
	}
	...
	if httpResponse.StatusCode < http.StatusOK || httpResponse.StatusCode >= http.StatusMultipleChoices {
		...
		return fmt.Errorf("managed carrier returned %d: %s", httpResponse.StatusCode, message)
	}
	if responseBody == nil {
		return nil
	}
	...
	if err := decoder.Decode(responseBody); err != nil {
		return fmt.Errorf("decode managed carrier response: %w", err)
	}
	return nil
}
