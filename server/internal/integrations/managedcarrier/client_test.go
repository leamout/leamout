...
func TestNewClientRequiresDeploymentCredentials(t *testing.T) {
	if _, err := NewClient(Config{BaseURL: "https://managed.example.test", DeploymentID: "deployment-123"}); err == nil {
		t.Fatal("expected missing token error")
	}
	if _, err := NewClient(Config{BaseURL: "https://managed.example.test", Token: "token"}); err == nil {
