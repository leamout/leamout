...
func ProvisionDIDWWIngress(ctx context.Context, cfg config.Config) error {
	if strings.TrimSpace(cfg.DIDWW.APIKey) == "" {
		return fmt.Errorf("DIDWW_API_KEY is required")
	}
	if strings.TrimSpace(cfg.DeploymentID) == "" {
		return fmt.Errorf("LEAMOUT_DEPLOYMENT_ID is required")
	}
	if strings.TrimSpace(cfg.SIP.PublicHost) == "" {
		return fmt.Errorf("SIP_PUBLIC_HOST is required")
	}
	...
	if len(sourceCIDRs) == 0 {
		return fmt.Errorf("DIDWW_SOURCE_CIDRS is required")
	}
	...
	if err != nil {
		return fmt.Errorf("connect provisioning database: %w", err)
	}
	defer db.Close()
	if err := db.Ping(ctx); err != nil {
		return fmt.Errorf("ping provisioning database: %w", err)
	}

	var providerID uuid.UUID
	if err := db.QueryRow(ctx, `SELECT id FROM carrier_providers WHERE slug=$1 AND status='active'`, didwwProviderSlug).Scan(&providerID); err != nil {
		return fmt.Errorf("resolve active %s carrier provider: %w", didwwProviderSlug, err)
	}
	...
	tx, err := db.Begin(ctx)
	if err != nil {
		return fmt.Errorf("begin DIDWW ingress provisioning: %w", err)
	}
	...
	if err := tx.Commit(ctx); err != nil {
		return fmt.Errorf("commit DIDWW ingress provisioning: %w", err)
	}
	return nil
}
