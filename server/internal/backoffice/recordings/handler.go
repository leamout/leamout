...
func (h *Handler) detail(w http.ResponseWriter, r *http.Request) {
	id, err := uuid.Parse(chi.URLParam(r, "recording_id"))
	if err != nil {
		http.NotFound(w, r)
		return
	}
	...
	if err != nil {
		http.Error(w, "load recordings detail", http.StatusInternalServerError)
		return
	}
...
func render(w http.ResponseWriter, r *http.Request, view templ.Component) {
	...
	if err := view.Render(r.Context(), &body); err != nil {
		http.Error(w, "render recordings view", http.StatusInternalServerError)
		return
	}
