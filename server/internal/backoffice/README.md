...
Current resource modules are `users`, `organizations`, `calls`, `numbers`,
`trunks`, `carrierconnections`, `providers`, and `commercial`. Each provides
cross-tenant list and detail projections for safe operator inspection.
