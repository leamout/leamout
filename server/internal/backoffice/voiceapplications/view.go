...
func listProps(items []VoiceApplication) components.ResourceListProps {
	...
	for i, v := range items {
		rows = append(
			...
			[]string{
				v.Name,
				v.OrganizationName,
				fmt.Sprint(v.BindingCount),
				fmt.Sprint(v.RingTimeoutSeconds) + "s",
				v.Status,
				v.CreatedAt,
			},
		)
	...
	return components.ResourceListProps{
		...
		Eyebrow:     "Telecom",
		Description: "Inspect call control applications, routing bindings, callbacks, and lifecycle state.",
		Active:      "voiceapplications",
		Headers:     []string{"Application", "Organization", "Bindings", "Ring timeout", "Status", "Created"},
		Rows:        rows,
		Links:       links,
	}
}
...
func detailProps(v Detail) components.ResourceDetailProps {
	return components.ResourceDetailProps{
		Title:     v.Name,
		Eyebrow:   "Voice application",
		Active:    "voiceapplications",
		BackLabel: "Voice Applications",
		BackHref:  "/voice-applications/",
		ID:        v.ID,
		Status:    v.Status,
		Stats: []components.ResourceField{
			{Label: "Bindings", Value: fmt.Sprint(v.BindingCount)},
			{Label: "Ring timeout", Value: fmt.Sprint(v.RingTimeoutSeconds) + " seconds"},
		},
		Sections: []components.ResourceSection{
			{
				Title: "Routing",
				Fields: []components.ResourceField{
					{Label: "Caller ID", Value: v.CallerID},
					{Label: "Voice URL", Value: v.VoiceUrl},
					{Label: "Callback URL", Value: v.CallbackUrl},
				},
			},
			{
