...
func (h *Handler) detail(w http.ResponseWriter, r *http.Request) {
	id, err := uuid.Parse(chi.URLParam(r, "connection_id"))
	if err != nil {
		http.NotFound(w, r)
		return
	}
	connection, err := h.repository.Get(r.Context(), id)
	if errors.Is(err, pgx.ErrNoRows) {
		http.NotFound(w, r)
		return
	}
	if err != nil {
		http.Error(w, "load carrier connection", 500)
		return
	}
	render(w, r, DetailPage(connection))
}
