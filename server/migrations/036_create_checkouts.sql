CREATE TABLE IF NOT EXISTS checkouts (
    ...
    CONSTRAINT uq_checkouts_payment_terms UNIQUE (id, organization_id, provider, amount_minor, currency),
    CONSTRAINT fk_checkouts_wallet_terms
        FOREIGN KEY (wallet_id, organization_id, currency)
        REFERENCES wallets (id, organization_id, currency)
        ON DELETE RESTRICT,
    CONSTRAINT fk_checkouts_price_currency
    ...
    CONSTRAINT chk_checkouts_provider CHECK (provider IN ('stripe', 'paystack')),
    CONSTRAINT chk_checkouts_method CHECK (
    ...
    CONSTRAINT chk_checkouts_terminal_action CHECK (
...
CREATE INDEX IF NOT EXISTS idx_checkouts_organization_created
    ON checkouts (organization_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_checkouts_pending_expiry
