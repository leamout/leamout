...
`deploy/self-hosted/opensips/drain.cfg` reserves global flag position `0` for admission drain state and exposes it through a local-only MI FIFO under `/run/opensips`.
...
```sh
./deploy/self-hosted/scripts/drain.sh
```
...
```sh
./deploy/self-hosted/scripts/resume.sh
```
